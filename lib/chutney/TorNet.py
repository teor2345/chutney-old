...
class TorEnviron(chutney.Templating.Environ):
    ...
    def _get_server_dns_resolv_conf(self, my):
        ...
        if dns_conf is None:
            dns_conf = ''
            sdrc_line = 'ServerDNSResolvConfFile /path/to/resolv.conf'
        else:
            dns_conf = os.path.abspath(dns_conf)
            sdrc_line = 'ServerDNSResolvConfFile %s' % (dns_conf)
        # if there is no DNS conf file set
        if my['dns_conf'] is None:
            return '#$CHUTNEY_DNS_CONF not set\n#' + sdrc_line
        else:
            ...
            if os.path.exists(dns_conf):
                ...
                print("ServerDNSResolvConfFile '%s' does not exist." % (
                          dns_conf))
